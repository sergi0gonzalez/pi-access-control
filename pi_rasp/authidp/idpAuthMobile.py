...
REQUEST_TOKEN_URL = 'http://identity.ua.pt/oauth/request_token?oauth_callback=oob'
...
GET_INFO_DATA_URL = 'http://identity.ua.pt/oauth/get_data?format=JSON&scope=student_info'
...
class UniversityOauth(object):
    ...
    def get_nmec(self):
        ...
        oauth = OAuth1Session(self.consumer_key, self.consumer_secret, resource_owner_key = self.access_token, resource_owner_secret = self.access_token_secret)
        r = oauth.get(GET_INFO_DATA_URL)
        dic = ast.literal_eval(r.content.decode("utf-8"))
        print(dic['NewDataSet']['ObterDadosAluno']['NMec'])
